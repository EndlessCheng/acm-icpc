...
{
	mem(fa, -1);
	int i, ans = 0, last, now;
	double alp;
	For(i, n) // 以这个起点开始的ok？
	{
		if ((alpha[i] - alpha[0]) * k + eps > 2 * pi) break;
		last = i;
		do
		{
			last = find(last);
			now = Lpos(alpha, n << 1, alp = alpha[i] + 2 * pi * (last - i + 1) / k);
			if (fabs(alp - alpha[now]) > eps) break; // 未找到
			fa[last] = now, last = now;
		}
		while (last - i + 1 < k);
		if (last - i + 1 >= k) ++ans;
	}
...
int main()
{
	...
	while (SI(n), n)
	{
		...
		sort(alpha, alpha + n);
		For(i, n) alpha[i + n] = alpha[i] + 2 * pi;
		Forr(k, 3, n + 1)
		{
			ans = solve(k);
			if (ans) PII(k, ans);
