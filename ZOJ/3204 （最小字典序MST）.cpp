...
struct edge
{
	...
	{
		return cost < b.cost || cost == b.cost && p2(from, to) < p2(b.from, b.to);
	}
};
vector<edge> e;
...
bool Kr(int n)
{
	...
	For(i, e.size())
	{
		fau = find(e[i].from), fav = find(e[i].to);
		if (fau != fav) fa[fau] = fav, ans.PB(p2(e[i].from, e[i].to)), ++cnt;
	}
	sort(all(ans));
	return cnt == n - 1;
}
...
int main()
{
	...
	TT
	{
		...
		{
			SI(cost);
			if (cost) e.PB(edge(i, j, cost));
		}
		if (Kr(n))
		{
			For(i, n - 2) printf("%d %d ", ans[i].x, ans[i].y);
			PII(ans[n - 2].x, ans[n - 2].y);
		}
		else puts("-1");
		//PI(ans.size());
	}
