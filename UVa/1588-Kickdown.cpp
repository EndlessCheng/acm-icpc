...
{
	char *p1 = s1, *p2 = s2;
	int l1 = strlen(p1), l2 = strlen(p2), i, j, ans;
	if (l1 > l2) swap(p1, p2), swap(l1, l2);
	For(i, l2 - l1 + 1)
	{
		For(j, l1) if (p1[j] + p2[i + j] == err) break;
		if (j == l1) return l2;
	}
	/// p1左移
	Forr(i, 1, l1)
	{
		for (j = 0; p1[i + j]; ++j) if (p2[j] + p1[i + j] == err) break;
		if (p1[i + j] == 0) {ans = i + l2; break;}
	}
	/// p1右移
	rFor(i, l1 - 2)
	{
		For(j, i + 1) if (p2[l2 - j - 1] + p1[i - j] == err) break;
		if (j == i + 1) return min(ans, l1 - i - 1 + l2);
	}
	return l1 + l2;
}
