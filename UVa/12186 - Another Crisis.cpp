...
#define Accv(a, n) (accumulate(a.begin(), a.begin() + (n), 0)) /// 注意0LL!!!!!以及0.0!!!
...
inline int Qceil(int x, int y) {return (x - 1) / y + 1;} /// 快速上取整，注意保持类型相同
...
int f(int u)
{
	...
	vector<int> d;
	int i;
	For(i, g[u].size()) d.PB(f(g[u][i]));
	return sort(all(d)), Accv(d, Qceil(g[u].size() * t, 100));
}
