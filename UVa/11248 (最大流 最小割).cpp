...
#define rForr(i, n, en) for (i = n; i >= (en); --i)
#define rForrr(i, n, en, step) for (i = n; i >= (en); i -= step)
#define loop(it, a) for (it = a.begin(); it != a.en(); ++it)
...
#define Fd(a, x) (find(all(a), x) != a.en())
...
struct ISAP
{
	int n, m, s, t;
	vector<Edge> edges;
	vector<int> G[mx]; // 邻接表，G[i][j]表示结点i的第j条边在e数组中的序号
	bool vis[mx]; // BFS使用
	int d[mx]; // 从起点到i的距离
	int cur[mx]; // 当前弧指针
	int p[mx]; // 可增广路上的上一条弧
...
int main()
{
	...
	while (scanf("%d%d%d", &n, &e, &c), n)
	{
		...
		while (e--)
		{
			...
			gr.AddEdge(b1 - 1, b2 - 1, fp);
		}
		int flow = gr.Maxflow(0, n - 1, inf);
		if (flow >= c) puts("possible");
		else
		{
			vector<int> cut = gr.Mincut();
			gr.Reduce();
			vector<Edge> ans;
			for (int i = 0; i < cut.size(); i++)
			{
				Edge &e = gr.edges[cut[i]];
				e.cap = c;
				gr.ClearFlow();
				if (flow + gr.Maxflow(0, n - 1, c - flow) >= c) ans.push_back(e);
				e.cap = 0;
			}
