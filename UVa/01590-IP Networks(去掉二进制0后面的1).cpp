...
ui calcmask()
{
	...
	for (Tmp = mask, mask = -1; Tmp; Tmp >>= 1) mask &= ~Tmp; /// 去掉二进制0后面的1
	return mask;
}
...
int main()
{
	...
	while (~SI(n))
	{
		...
		while (n--) For(i, 4)
		{
			scanf("%uc", &tmp[i]), getchar();
			And[i] &= tmp[i], Or[i] |= tmp[i];
		}
		mask = calcmask();
		For(i, 4) ans[i] = (mask >> ((3 - i) << 3)) & 255;
		For(i, 4) printf(i == 3 ? "%d\n" : "%d.", ans[i] & tmp[i]);
		For(i, 4) printf(i == 3 ? "%d\n" : "%d.", ans[i]);
	}
	return 0;
}
