...
template<class T>
inline void Qmin(T &a, const T b) { if (b < a) a = b; }

template<class T>
inline void Qmax(T &a, const T b) { if (a < b) a = b; } // *若考虑位置，加上等号
...
template<class T>
inline T Qceil(T x, T y) { return (T) ceil(double(x) / y - 1e-8); }
...
#define Pcas() printf("Test case #%d: \n", ++cas) // *注意C的大小写，空输出注意去空格
int cas;
const int mx = 205;
...
struct Line {
    ...
    bool operator<(const Line &b) const { return h < b.h; }
} line[mx << 1];

double sum[mx << 2], x_pos[mx];
int cov[mx << 2];

void push_up(int l, int r, int rt) {
    if (cov[rt]) sum[rt] = x_pos[r + 1] - x_pos[l];
...
int main() {
    ...
    while (SI(n), n) {
        ...
        For(i, n) {
            ...
            if (mp.find(x1) == mp.end()) mp[x1] = cnt;
            line[cnt++] = Line(x1, x2, y2, -1);
            if (mp.find(x2) == mp.end()) mp[x2] = cnt;
        }
        sort(line, line + cnt);
        int k = 0;
        loop(it, mp) {
        ...
        For(i, cnt - 1) {
            int l = mp[line[i].l];
            int r = mp[line[i].r] - 1;
            update(line[i].f, l, r, 1, cnt, 1);
            ans += (line[i + 1].h - line[i].h) * sum[1];
        }
